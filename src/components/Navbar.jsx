  ...
  return (
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between items-center max-w-7x1 mx-auto'>
        ...
        >
          <img src={SternizBg} alt='logo.png' className='w-19 h-12 object.contain' />
          {/* <p className='text-white text-[18px] font-bold cursor-pointer'>Burhan <span className='sm:block hidden'> Ahmad</span></p> */}
        </a>
        ...
        <ul className='list-none hidden sm:flex flex-row gap-10'>
          {navLinks.map((link) => (
            ...
            >
              <a href={link.url}>{link.title}</a> {/* Changed from `#id` to `url` */}
            </li>
          ))}
        </ul>
